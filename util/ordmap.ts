import * as RBT from "./redblack.ts";
import { LessThan, numberLT, stringLT } from "./shared.ts";
import { ForwardIterator, ReverseIterator } from "./redblack-iterators.ts";
...
export class OrdMap<k, v> {
    ...
    insert(key: k, value: v): OrdMap<k, v> {
        return new OrdMap(
            this.compare,
            this.root.insert(this.compare, key, value),
        );
    }
    ...
    difference(other: OrdMap<k, v>): OrdMap<k, v> {
        ...
        newMap = this.foldl(
            (
                ...
                key,
                val,
            ) => (other.find(key) === undefined ? map.insert(key, val) : map),
            newMap,
        );
        ...
        newMap = other.foldl(
            (
                ...
                key,
                val,
            ) => (this.find(key) === undefined ? map.insert(key, val) : map),
            newMap,
        );
...
function checkComparisonFuncEquality<a>(
    f1: LessThan<a>,
    f2: LessThan<a>,
): void {
    if (f1 !== f2) {
