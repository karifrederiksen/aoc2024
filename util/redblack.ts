...
import { ForwardIterator } from "./redblack-iterators.ts";
...
export function foldl<k, v, b>(
    n: Node<k, v>,
    f: (curr: b, nextKey: k, nextVal: v) => b,
    initial: b,
): b {
...
function balRight<k, v>(
    ...
    if (tl.color === Color.Black) {
        return balance(
            x,
            xv,
            new NonEmptyNode(tl.key, tl.value, tl.left, tl.right, Color.Red),
            tr,
        );
    }
...
function append<k, v>(
    ...
    throw "unmatched tree on append: " + JSON.stringify(tl) + ", " +
        JSON.stringify(tr);
}
...
export function checkInvariants<a extends NodeUnion<unknown, unknown>>(
    tree_: a,
): void {
